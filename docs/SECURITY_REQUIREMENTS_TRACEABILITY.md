...
| Audit/WORM | HMAC chain, externally deliverable signed checkpoints, đo chính xác tail chưa neo/freshness; high profile bắt buộc neo từng sự kiện và không chấp nhận tail chưa neo, append-only DB grants | audit/checkpoint freshness tests | WORM/retention-lock receiver, SOC alert/rule ownership |
