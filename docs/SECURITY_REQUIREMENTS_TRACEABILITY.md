...
| Export an toàn | `StreamingResponse`, paging từng dòng, step-up, ticket 60 giây single-use; UI không ghi plaintext temp | `test_high_security_features.py` | endpoint HTTPS, browser/endpoint monitoring |
...
| Consent AI | timestamp + policy version, chỉ context sau consent, tối đa 8 message | API/provider tests | nội dung thông báo pháp lý được phê duyệt |
...
| Gradio/HTTP | API JWT/RBAC, high profile OIDC proxy gate, upload cap, CSP enforce + stricter report-only, headers | hardening/UI tests | OIDC IdP/proxy thật, nonce/hash CSP nếu Gradio hỗ trợ |
...
| Retention | mode-specific expiry, startup sweep, scheduler command, wrapped-DEK deletion | retention tests | backup/snapshot lifecycle và legal hold |
